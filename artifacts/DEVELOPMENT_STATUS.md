...
Updated: Package renamed to `com.strawberryFrappe.sync_companion`. Foreground service changed to `dataSync` type for Android 16 compatibility. Service runs in main process now (no separate `:ble_service` process).
